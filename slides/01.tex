...
\begin{frame}
  ...
  \centering
  \vskip 11pt
  \structure{Performance} = How fast the CPU \emph{retires} instructions.
  \vskip 11pt
  \pause
  \begin{challenge}{Mismatch}
    Programmers and hardware designers see instructions differently.
  \end{challenge}
\end{frame}
...
\begin{frame}
  \frametitle{Stored-programme architecture today}
  ...
  \onslide<3>{\structure{ONE FEATURE:} both instructions and data reside in memory.}
\end{frame}
...
\begin{frame}
  ...
  \begin{tabular}{ll}
    ...
    \structure{Latency} & \# cycles to execute instruction\\[11pt]
    \structure{Throughput} & \# instructions that can run simultaneously
  \end{tabular}
\end{frame}
...
\end{minted}
    ...
    \begin{column}{0.6\textwidth}
      \only<1>{Scalar addition, 1 output element per instruction.
        \begin{center}
          \includegraphics[width=0.9\textwidth]{figures/scalaradd.png}
        \end{center}}
      \only<2>{AVX addition, 4 output elements per instruction.
        \begin{center}
          \includegraphics[width=0.9\textwidth]{figures/vectoradd.png}
        \end{center}}
    \end{column}
  \end{columns}
\end{frame}
